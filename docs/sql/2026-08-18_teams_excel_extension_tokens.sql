-- ============================================================
-- WebGantt: Teams共有Excel連携（ブラウザ拡張機能方式）
-- マイグレーション: teams_excel_extension_tokens テーブル作成
-- ============================================================
-- 参照: docs/teams-excel-import-design.md 7.1節・10.1節・11.1節
-- 実行方法（本番サーバー）:
--   mysql -u root -p gantt_collab < docs/sql/2026-08-18_teams_excel_extension_tokens.sql
...
-- google_calendar_tokens（2026-08-15）・office_calendar_tokens（2026-08-17）と
-- 同一の命名・型規約を踏襲している。
--
-- 設計書11.1節（コード分離方針）: 本機能はサーバー側で「特殊（汎用性は低い）」
-- 機能として明示的に分離することが確定要望のため、テーブル名は
-- teams_excel_ プレフィックスで統一し、将来この機能を廃止する場合は
-- 「api/teams_excel_import.php の削除」「本テーブル群のDROP TABLE」
...
--
-- ロールバック（機能を撤去する場合）:
--   DROP TABLE IF EXISTS teams_excel_extension_tokens;
-- ============================================================

CREATE TABLE IF NOT EXISTS teams_excel_extension_tokens (
  ...
  CONSTRAINT fk_teet_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
