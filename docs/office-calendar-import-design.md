...
1. ユーザー（d-takeda様）の会社アカウント（`REDACTED-EMAIL`）は、組織ポリシーによりAzure Portal管理画面へのアクセスが完全にブロックされている（401エラー）
...
3. しかし、実機検証（`Calendars.Read`委任権限を追加し、d-takeda様の会社アカウントで実際に認可URLを開くテスト）を行った結果、**「管理者の承認が必要」という画面が表示され、一般ユーザー自身の同意では連携できないことを確認した**
...
- 実機検証の結果、d-takeda様の会社アカウントでこの機能（Outlook on the web → 設定 → 予定表 → 共有予定表 → 予定表を公開する）が組織ポリシーでブロックされておらず、**ICS購読URLの発行に成功した**ことを確認済み
...
1. Azure AD OAuth方式は、御社テナントの`risk-based step-up consent`ポリシーにより一般ユーザーの同意がブロックされ、「管理者の承認が必要」画面が表示されることを実機（`REDACTED-EMAIL`、Azureポータル上のアプリ登録画面経由の認可URL）で確認
...
- 実機検証（`REDACTED-EMAIL`で認可URLを開く）の結果、「管理者の承認が必要」画面が表示され、一般ユーザーの同意では完結しないことを確認 → **この方式は不採用**
