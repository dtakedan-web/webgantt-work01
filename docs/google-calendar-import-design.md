...
- 作成日: 2026-08-14
- ステータス: **確定・ユーザー承認済み（2026-08-14、UI名称・サブメニュー構成の追加要望も反映済み）／2026-08-15: 実装着手前調査によりAPI設計を方針転換（6節・8.2節、タスク生成はフロントエンドJS側で実施）、ユーザー承認済み。クライアントID/シークレット発行済み。実装フェーズ進行中／2026-08-16: 実装完了・実機（Windows/Chrome）でのGoogleカレンダー連携動作確認・ユーザー最終承認済み。現在はOAuth同意画面が「テスト」ステータスのまま運用中（テストユーザー登録者のみ利用可）。本番公開（選択肢B、不特定多数への公開）は将来対応として14節に記録**
...
**上記1〜8は全て完了。2026-08-16、ユーザーが実機で「一通り、Googleカレンダーからインポートできることも確認しました」と最終承認し、Task Bは実装・検証・承認まで完結した。**

---

## 14. 本番公開に向けた対応（将来作業・選択肢B、未着手）

### 14.1 背景・現状

現在のGoogle Cloud ConsoleのOAuth同意画面は、12.2節の手順どおり **Publishing Status: 「テスト」** のまま運用している。この状態では、事前にテストユーザーとして登録した**最大100アカウント**のGoogleアカウントのみが本機能で連携可能であり、それ以外の一般ユーザー（テストユーザー未登録の個人Gmailアカウント等）は、Google側の認可画面で「このアプリはGoogleで確認されていません」という警告が表示されるか、そもそもアクセスを拒否される（12.3節既述）。

2026-08-16、ユーザーより「テスト完了後、一般公開する場合のGoogle Cloud側の設定はどうすればよいか」という質問があり、調査の結果、以下2つの選択肢があることを確認した。

### 14.2 選択肢A: Internal（組織内限定公開、審査不要）

- 利用者がGoogle Workspace／Cloud Identity組織に所属するメンバーのみに限定される場合に選択可能
- OAuth同意画面の **User Type** を「Internal（内部）」に設定することで、Googleによる本番公開審査（Verification）が一切不要になる
- ただし、組織に所属しない個人Gmailアカウント等では利用不可

### 14.3 選択肢B: 本番公開（不特定多数への公開、審査必要）— **ユーザー確定方針（2026-08-16）**

個人Gmailアカウントを含む不特定多数の一般ユーザーに公開する場合は、Googleの **アプリ確認審査（App Verification）** を受ける必要がある。本機能が使用するスコープ `https://www.googleapis.com/auth/calendar.readonly` は **Sensitive scope（機微スコープ）** に分類され（Restricted scopeよりは審査要件が軽く、年次のセキュリティ監査は不要）、以下の対応が必要となる。

参考: `https://developers.google.com/identity/protocols/oauth2/production-readiness/sensitive-scope-verification`

#### 14.3.1 必要な準備事項

1. **ブランド確認（Branding）**
   Cloud Console「OAuth同意画面」→「ブランディング」ページで以下を設定・確認する:
   - アプリ名
   - アプリのロゴ
   - サポートメールアドレス
   - プライバシーポリシーのURL（公開されている必要あり）
   - ホームページURL（公開されている必要あり）

2. **機微スコープ確認（Sensitive Scope Verification）**
   Verification Center経由で以下を提出する:
   - `calendar.readonly`スコープを要求する正当な理由の説明文（本機能の目的: Googleカレンダーの予定をWebGanttのタスクとして読み取り専用でインポートするため、と説明）
   - 認可フロー開始から実際の機能利用までの一連の操作を録画した**英語のデモ動画**を作成し、YouTubeに**限定公開（Unlisted）**でアップロードしてURLを提出する

3. **審査期間**
   提出後、審査完了まで**最大10日程度**を要する見込み（Googleの審査状況により変動）

#### 14.3.2 対応方針・スケジュール

- ユーザー確認の結果、**選択肢Bで進める方針を確定**した（2026-08-16）
- ただし「すぐには対応できない」とのことで、**本対応は将来の作業として保留**する
- 保留中は現状の「テスト」ステータス運用を継続し、利用したいGoogleアカウントは都度テストユーザーとして追加登録する（12.2節手順4番目参照）
- 着手時期が決まり次第、本節を起点にブランド確認・デモ動画作成・Verification Center申請を進める

#### 14.3.3 着手時のチェックリスト（将来作業用）

- [ ] Cloud Console「ブランディング」ページでアプリ名・ロゴ・サポートメール・プライバシーポリシーURL・ホームページURLを設定
- [ ] プライバシーポリシーページ・ホームページを実際に公開（URLが到達可能な状態にする）
- [ ] `calendar.readonly`スコープの使用理由説明文を作成（英語）
- [ ] 認可フロー〜機能利用までのデモ動画を録画（英語ナレーション/字幕）
- [ ] デモ動画をYouTubeに限定公開でアップロード
- [ ] Cloud Console「OAuth同意画面」からVerification Center経由で審査を申請
- [ ] 審査完了まで待機（目安10日程度）、Publishing Statusを「本番」に変更
